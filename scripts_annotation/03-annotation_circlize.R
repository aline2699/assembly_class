...
gff_file <- "/data/users/asteiner/assembly_annotation_course/EDTA/results/EDTA_annotation/ERR11437351.asm.bp.p_ctg.fa.mod.EDTA.TEanno.gff3"
gff_data <- read.table(gff_file, header = FALSE, sep = "\t", stringsAsFactors = FALSE)

gff_annotation <- "/data/users/asteiner/assembly_annotation_course/gene_annotation/final/filtered.genes.renamed.gff3"
...
custom_ideogram <- read.table("/data/users/asteiner/assembly_annotation_course/samtools/results/hifiasm_assembly.fai", header = FALSE, stringsAsFactors = FALSE)
...
pdf("R_visualizations/plots/02-TE_density_with_genes.pdf", width = 10, height = 10)
...
lgd <- Legend(
    title = "Superfamily", at = c("gene", "Gypsy_LTR_retrotransposon", "Copia_LTR_retrotransposon", "tRNA_SINE_retrotransposon", "L1_LINE_retrotransposon", "Mutator_TIR_transposon"),
